...
async fn main() -> Result<()> {
    ...
    let mut last_indexed_block = get_or_create_cursor(&pool, contract_address, &provider).await?;
    info!(last_indexed_block, "Starting indexer at block height ...");
    loop {
        match sync_events(&pool, contract_address, &provider, last_indexed_block).await {
